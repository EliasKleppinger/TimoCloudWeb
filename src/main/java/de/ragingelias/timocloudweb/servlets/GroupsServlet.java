			...
			if (req.getParameter("edit") != null) {
				...
				if (statusMessage.contains("edit_success")) {
					...
					for (ServerGroupObject server : TimoCloudAPI.getUniversalAPI().getServerGroups()) {
						builder += "<div class='group'><p>Name: " + server.getName() + "</p>" + "<p>RAM: "
								+ server.getRam() + " MB</p>" + "<p>Online: " + server.getOnlineAmount() + "</p>"
								+ "<p>Static: " + server.isStatic() + "</p><a href='?edit=" + server.getName()
								+ "'><button>Editieren</button></a></div>";
					}

					resp.setContentType("text/html");
					resp.setStatus(HttpServletResponse.SC_OK);
					resp.getWriter().println(TimoCloudWeb.navbar
							+ "<div class='box'><h3 style='color: red;'>Die Gruppe wurde erfolgreich editiert!</h3><h2>Gruppen</h2>"
					...
					resp.setStatus(HttpServletResponse.SC_OK);
					resp.getWriter().println(TimoCloudWeb.navbar
							+ "<div class='box'><h3 style='color: red;'>Die Gruppe wurde erstellt!</h3><h2>Gruppen</h2>"
							+ builder + "<br><br><a href='?add=true'><button>Gruppe erstellen</button></a></div>");
				}
				...
				if(req.getParameter("addstatus") != null) {
					 ...
					 boolean adding = Boolean.valueOf(req.getParameter("add"));
						if (adding) {
							...
							resp.setStatus(HttpServletResponse.SC_OK);
							resp.getWriter().println(TimoCloudWeb.navbar
									+ "<div class='box'><h3 style='color: green;'>" + statusmessage + "</h3><h2>Gruppe Erstellen</h2>" + form + "</div>");
						} else {
							resp.sendRedirect("/groups");
						}
				} else {
